...
APP_VERSION = os.getenv("APP_VERSION", "2026-05-14-concept-cross-guideline-artifacts-v29")
...
- 背景知識可能包含 compiled guideline artifacts，這些是從已上傳 Markdown 指南預先編譯出的 recommendation/table/section/concept/cross-guideline 證據卡；它們可用來快速定位證據，但若有父層章節或原始片段，仍以原始 Markdown 指南內容為準。
- 不要使用模型內建知識、一般醫學常識、未載入指南、新聞或推測補完。
- 若問題屬於糖尿病、CKD、高血壓、血脂、心血管風險、肥胖、脂肪肝或慢性病照護範圍，而且指南片段有相關依據，請回答片段能支持的部分；若證據不完整，請說明限制，不要直接拒答。
- 只有在問題明顯離開上述慢性病照護範圍，或本輪完全沒有相關指南片段時，才說目前指南知識庫資料不足。
...
def selected_guideline_evidence_present(hits: list[KnowledgeHit]) -> bool:
    for hit in hits:
        ...
        if re.search(r"\b(reference|references|acknowledg)\b", haystack) and not re.search(
            ...
            continue
        if re.search(r"\b(ada standards|aace|kdigo|dc26s\d+|recommendation|practice point|table|section map|treatment|goal|target|diagnosis|screening|management)\b", haystack):
            return True
    return False
...
def select_guideline_hits(
    ...
    if not local_answerable and answerable_with_available_guideline_evidence(user_text, selected, clinical_intent):
        ...
        coverage_gaps = (
            coverage_gaps + "；" if coverage_gaps else ""
        ) + "本題屬於已載入指南涵蓋的慢性病照護範圍；雖然部分 facet 未完全命中，仍可根據已選指南片段回答可支持的部分並標示限制。"
    if not local_answerable:
        answerable = False
...
def llm_answer(user_text: str, line_user_id: str = "") -> str:
    ...
    if selected_hits:
        ...
        if recursive_answerable:
            ...
            coverage_gaps = (
                coverage_gaps + "；" if coverage_gaps else ""
            ) + "本題屬於已載入指南涵蓋的慢性病照護範圍；以下回答會限制在目前檢索到的指南片段。"
        elif recursive_gap:
